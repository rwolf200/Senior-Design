...
def build_labels_for_split(root: str, split: str = "train"):
    """
    Build a list of (image_path, color_list) entries for a given split.

    The expected directory layout is:
        root/split/<color>/*.ext
        root/split/colorless/*.ext
    ...
    Returns:
        list[tuple[str, list[str]]]: List of (path, [colors]) pairs.
                                     Empty list means colorless.
    """
    split_root = os.path.join(root, split)
    fname_to_colors = {}

    # Collect filenames and associated colors
    for color in COLORS:
    ...
    # Colorless images (present in colorless folder only)
    ...
    # Resolve each filename to a concrete path and sorted color list
    entries = []
    for fname, color_set in fname_to_colors.items():
        chosen_path = None
        if color_set:
            # Multi- or single-colored: pick the first existing path
            ...
            # Colorless: look in colorless folder
...
class MTGColorDataset(Dataset):
    """
    Dataset for MTG color classification.

    Each sample returns an image tensor and a multi-hot label vector of length
    NUM_CLASSES, where the last index corresponds to colorless.
    """

    def __init__(self, entries, transform=None):
        """
        Args:
            entries (list): List of (path, color_list) pairs.
...
def find_best_thresholds(all_trues, all_probs, step=0.01):
    """
    Find per-class thresholds that maximize F1 for W/U/B/R/G.

    Colorless uses a fixed threshold of 0.5.
    ...
    Returns:
        list[float]: Thresholds for each class (W, U, B, R, G, colorless).
    """
    all_trues = np.asarray(all_trues)
    all_probs = np.asarray(all_probs)
    best_thresholds = []

    # Tune thresholds for W/U/B/R/G only
    ...
    # Colorless threshold fixed at 0.5
...
def letters_from_logits_with_thresholds(logits, thresholds):
    """
    Convert logits to color letters using per-class thresholds.
    ...
    Returns:
        list[str]: List of color letters (e.g. ["W", "U"]). Empty means colorless.
    """
...
def train(args):
    """
    Train the multi-label MTG color classifier.
...
def evaluate(model, dataloader, device, thresholds=None):
    ...
    # Per-class metrics for W/U/B/R/G
...
def sort_wubrg(colors):
    """
    Sort a list of color letters in WUBRG order.

    Args:
        colors (list[str]): List of color letters.

    Returns:
        list[str]: Sorted list in WUBRG order.
    """
...
def test_scryfall(args, n_tests=5000):
    ...
    downloads the image, runs the model, and compares predicted colors
    to Scryfall's color_identity.
